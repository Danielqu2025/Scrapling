"""Convert Fengxian list API rows to disclosure events."""
...
from sources.district import DISTRICT_LABELS, DISTRICT_FENGXIAN, STAGE_ACCEPTANCE
from sources.types import SOURCE_DISTRICT_FENGXIAN
...
def _source_url(stage: str) -> str:
    base = "http://211.136.184.90:8090"
    return f"{base}/acceptance" if stage == STAGE_ACCEPTANCE else f"{base}/public"


def fengxian_row_to_event(stage: str, row: dict[str, Any], *, page_no: int = 1) -> dict[str, Any]:
    ...
    approval_number = str(row.get("PIWNENO") or "").strip()
    item_no = str(row.get("ITEMNO") or "").strip()
    row_id = str(row.get("ROW_ID") or "").strip()
    external_id = item_no or row_id or f"{project_name}|{_event_date(row)}"
    attachments = []
    ...
    return {
        ...
        "company": company,
        "location": address,
        "district": DISTRICT_LABELS.get(DISTRICT_FENGXIAN, "奉贤区"),
        ...
        "summary_json": {
            ...
            "district_code": DISTRICT_FENGXIAN,
        },
        "source_url": _source_url(stage),
        "page_no": page_no,
        "files": [],
    }


def fengxian_rows_to_events(stage: str, rows: list[dict[str, Any]], *, page_no: int = 1) -> list[dict[str, Any]]:
    return [fengxian_row_to_event(stage, row, page_no=page_no) for row in rows]
