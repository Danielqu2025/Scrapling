...
from _common import DISCLOSURE_TYPES, is_html_response, normalize_response_body, resolve_download_url
...
def _run_update_check(trigger_mode: str = "startup") -> dict[str, Any]:
    ...
    try:
        ...
        if result["has_updates"] and trigger_mode == "auto":
            if not sync_running:
                _run_sync_job(list(DISCLOSURE_TYPES), 1, "auto")
...
def _run_sync_job(disclosure_types: list[str], max_pages: int | None, trigger_mode: str) -> None:
    global sync_running
    with sync_lock:
        sync_running = True
    status = "failed"
...
def api_sync(request: SyncRequest, background_tasks: BackgroundTasks) -> dict[str, Any]:
    ...
    background_tasks.add_task(_run_sync_job, request.disclosure_types, max_pages, "manual")
    return {
        "status": "accepted",
